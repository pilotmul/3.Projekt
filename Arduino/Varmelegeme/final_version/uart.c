...
// Funktion til initialisering af UART0 med specificeret baud rate og antal databits s�ttes normalt til
// InitUART(9600,8,1);
void InitUART(unsigned long BaudRate, unsigned char DataBit, unsigned char Rx_Int)
{
	// Sikrer, at inputparametrene er inden for tilladte gr�nser
	if (BaudRate < 300 || BaudRate >= 115200 || DataBit < 5 || DataBit > 8) {
		return;  // Returnerer og afbryder funktionen, hvis parametre er ugyldige
	}
	
	//Starter interrupt baseret p� om Rx_Int er 0 eller 1.
	if (Rx_Int>0)
	{
		sei();
	...
	// Beregner v�rdien til UBRR0 registeret for at indstille baud raten
	unsigned int ubrrValue = (F_CPU / 16 / BaudRate) - 1;
	UBRR0H = (unsigned char)(ubrrValue >> 8);  // S�tter de h�jeste 8 bits af UBRR0
	UBRR0L = (unsigned char)ubrrValue;  // S�tter de laveste 8 bits af UBRR0

	UCSR0C = (1 << UCSZ01) | (1 << UCSZ00); // 8 databits, 1 stopbit, ingen paritet


	// Nulstiller f�rst UCSR0C og indstiller derefter de n�dvendige flags
	UCSR0C = 0;
	UCSR0C |= (1 << UCSZ00);  // S�tter basisindstillingerne for frame format: asynkron mode, 1 stop bit

	// Indstiller antallet af databits per frame baseret p� inputparameteren
	switch (DataBit) {
...
// Funktion til at l�se en karakter fra UART
char ReadChar()
{
...
{
	while (*Streng) {  // L�kke gennem hver karakter i strengen indtil null-terminator
...
char* float2Char(float val, char* buffer){
  ...
  sprintf (buffer, "%d.%d", val_int, val_fra);
  return buffer;
}